...
const mpBaseConfig = {
  entry: config.mp.entry,
  output: config.mp.output,
  mode: 'production',
  optimization: {
  ...
  plugins: [
    ...
    new MpPlugin(config.mp.mpPlugin),
    new CopyPlugin({patterns: config.mp.copy}),
    new webpack.DefinePlugin(config.mp.defines)
  ],
  module: {
    rules: [
      {
...
// @ts-ignore
export default merge(mpBaseConfig, config.commonWebpackConfig, mpCustomConfig);
